...
knitr::opts_chunk$set(echo = TRUE, fig.width = 12, 
                      fig.height = 8, warning=FALSE, set.seed (20211127))
library(tidyverse)
...
will drop those NA values. After the drop of the NA values the dimension of 
the tibble is $333 \times 8$.
```{r warning=FALSE}
...
plot_ly(penguins_df, x = ~bill_length_mm, y = ~bill_depth_mm, 
        ...
        marker = list(symbol = 'circle', 
                      sizemode = 'diameter'), 
        sizes = c(1, 20),
        color = ~sex, colors = c('#d8b365','#7fbf7b'))
```
...
of the PC. As can be seen the cumulativepercentage of expressed variance by 
the first two is about $80\%$.
```{r}
...
# Creating the model 
We will use two very simple model __logistic regression__ and a more complex
but less interpretable model __random forest__
...
__bill_depth_mm__the odds of beeing male vs beeing female increase of $7.23$ 
time.
```{r}
...
As can be seen the __flipper_length_mm__  and the __bill_length_mm_ are not
statistically significant. We can remove those predictors and train again
the model. 
...
__bill_depth_mm__ the odds of beeing male vs beeing female increase of $7.45$ 
time. While for each $1$ g increment in the __bill_depth_mm__ the odds of beeing 
